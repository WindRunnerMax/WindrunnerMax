...
**2022.04-2022.09　　　　　　　　　　上海随训通电子科技有限公司**  
**基础架构-字节云**    
实习期间主要工作为NoCode建站平台编辑器`SDK`的开发，主要负责了文档编辑器的建设以及NoCode编辑器功能区块的扩充，在职期间完成了团队项目的Monorepo迁移，实现了编辑器网格拖拽布局，补齐了编辑器近百个any的类型定义，解决了大量遗留BUG，参与制定了编辑器的联动规范与基础组件的实现。在业余时间基于开源的slate实现了 [文档编辑器](https://github.com/WindrunnerMax/DocEditor) 并发布为NPM包，并作为基于NoCode实现的 [简历编辑器](https://github.com/WindrunnerMax/ResumeEditor) 中富文本组件引用。

* 以编辑器为切入点，负责了编辑器能力建设、数据转换`Node`后端服务、编辑器对外`SDK`的开发。
* 完成了数据转换的能力，支持了DSL转Markdown、Markdown转DSL、HTML转DSL的功能，支持了火山引擎与飞书文档的导入。
* 完成了文档编辑器中图片、代码块、视频、高亮块、Mention组件、流程图、高亮块组件的开发，支持了标题组件拓展、表格合并拆分、评论功能，并通过数据转换序列化与反序列化实现了对飞书文档的复制粘贴，在体验方面对标飞书文档。
* 扩充了NoCode编辑器的轮播图与Tab组件，并在Tab组件的开发中参与制定了编辑器的联动规范，并基于此实现了可嵌套的栅格基础组件。
* 拆离了编辑器`SDK`作为独立依赖，通过`rollup`打包为`ESM`库，处理了副作用问题，完善了项目作为`Headless CMS`的能力。
* 完成了团队编辑器、渲染侧、管理侧项目的Monorepo迁移，其中主要负责了仓库迁移、冲突解决、依赖问题、项目编译脚本、编辑器部分的流水线部署与发布的相关能力。
* 重构了文档的工具栏，通过按行处理并使用React的事件绑定而不是主动操作DOM来解决了遗留已久的问题，并基于此实现了对于嵌套组件例如引用、表格等组件的按行编写，提高了可控性与稳定性并支持了新的feature。
* 通过对编辑器heading插件的重写，实现了文档的锚点功能，并实现锚点过多出现滚动条后始终显示在可视区域的优化。
* 实现了文档中图片组件的四向对角等比拖拽放大缩小与iframe组件的八向拖拽放大缩小，以及居中和对齐功能。
* 通过调研了不同的编辑器的拖拽能力，并结合之前代码的实现，完成了编辑器的网格拖拽布局能力，并实现了对于之前拖拽区块与组件的兼容，非破坏性的更改，极大提高了用户拖拽与对齐的体验。
* 为项目接入了commit lint、react-hooks lint、style lint，优化了TS lint的检查，规范了代码的提交。
* 处理了文档编辑器的core依赖组件升级的break changes，为后期更好的接入新开发的功能做好基础。
* 补齐了编辑器近百个any的类型定义，并借助TS的类型检查发现了一些不易察觉的问题，并为后期良好的静态类型检查打下基础。
* 解决了大量遗留BUG，诸如拖拽区块重叠时的剧烈抖动问题、菜单栏的命令指定定位错误问题、快速resize导致页面崩溃等等，提升了用户体验。
* 优化了编辑器的整体结构，通过对JSON编辑器与文档页面的优化使得BUG的调试调试更加简单。
* 实现了ToolTip组件，学习了在React中直接操作DOM结构与定位，思考了如何维持组件数量及位置的稳定性，对标飞书wiki的用户体验。
* 在使用同业务组的ArcoDesign组件库的过程中发现的问题，提出 #878 #880 两个PR并合并修复。
* 组内作了两次分享，分享了新人入职时编辑器数据流的流动与组件结构，分享了项目中解除复制限制脚本的相关内容。
...
**2020-06　至　今　　　　　　　　　　　文本复制脚本**
* 个人开发的油猴的脚本，使用ts+rollup构建，累计130万次安装
