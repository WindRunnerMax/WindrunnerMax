...
**基础架构-字节云**    
实习期间主要工作为NoCode建站平台编辑器部分的开发，主要负责了文档编辑器的建设以及NoCode编辑器功能区块的扩充，在职期间完成了团队项目的Monorepo迁移，实现了编辑器网格拖拽布局，补齐了编辑器近百个any的类型定义，解决了大量遗留BUG，参与制定了编辑器的联动规范与基础组件的实现。在业余时间基于开源的slate实现了 [文档编辑器](https://github.com/WindrunnerMax/DocEditor) 并发布为NPM包，并作为基于NoCode实现的 [简历编辑器](https://github.com/WindrunnerMax/ResumeEditor) 中富文本组件引用。
...
* 组内作了两次分享，分享了新人入职时编辑器数据流的流动与组建结构，分享了项目中解除复制限制脚本的相关内容。
